...
export const MAPPING_RULES = [
  { sor: 'load_number', source: 'load_no', transform: 'Trim whitespace; must match LD-<digits>' },
  { sor: 'bol_number', source: 'frt_ord_no', transform: 'Trim whitespace (Walmart calls the BOL the "Freight Order Number")' },
  { sor: 'shipper_name', source: 'shipper_nm', transform: 'Trim whitespace' },
  { sor: 'origin_city / origin_state', source: 'orig_city / orig_st', transform: 'Trim whitespace' },
  { sor: 'destination_city / destination_state', source: 'dest_city / dest_st', transform: 'Trim whitespace' },
  { sor: 'ship_date', source: 'shp_dt', transform: 'Reorder MMDDYYYY → DDMMYYYY (Walmart is month-first, the SOR is day-first)' },
  { sor: 'delivery_date', source: 'del_dt', transform: 'Reorder MMDDYYYY → DDMMYYYY' },
  { sor: 'weight', source: 'wgt', transform: 'Strip commas and the "lbs" unit label; send as a plain whole number (e.g. "41,860 lbs" → 41860)' },
  { sor: 'equipment_type', source: 'mode', transform: `AMBIENT → Dry Van 53' · REFRIGERATED or FREEZER → Reefer 53' (case/whitespace-insensitive)` },
];
...
export const FLAG_RULES = [
  {
    ...
    sopRef: 'SOP Step 13 / Edge case 1',
  },
  {
    id: 'ambiguous_mode',
    ...
    sopRef: 'SOP Step 14 / Edge case 2',
  },
  {
    id: 'missing_weight',
    title: 'Missing or unreadable weight',
    trigger: 'Tender weight is blank or cannot be parsed to a number',
    action: 'Do not build the load. All SOR fields are required — confirm the true weight with Walmart first.',
    sopRef: 'SOR API: all fields required',
  },
  {
    id: 'invalid_field',
