...
export const MAPPING_RULES = [
  ...
  { sor: 'Expected Ship Date', source: 'Ship Date (shp_dt)', transform: 'Transform from MMDDYYYY → DDMMYYYY (Walmart is month-first, the SHV is day-first)' },
  { sor: 'Expected Delivery Date', source: 'Delivery Date (del_dt)', transform: 'Transform from MMDDYYYY → DDMMYYYY (Walmart is month-first, the SHV is day-first)' },
  { sor: 'Total Weight (lbs)', source: 'Gross Weight (wgt)', transform: 'Transform by: Removing commas and the "lbs" unit label; send as a plain whole number (e.g. "41,860 lbs" → 41860). Flag weight above 45,000 lbs.' },
  { sor: 'Equipment Type', source: 'Mode (mode)', transform: `AMBIENT → Dry Van 53' · REFRIGERATED or FREEZER → Reefer 53'` },
];
